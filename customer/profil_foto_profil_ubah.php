<?php
    ...
    mysqli_stmt_prepare($stmt_customer, "SELECT id_customer, path_to_foto_profil FROM tbl_customer WHERE id_customer=?");
    ...
    if (!$customer) {
        $_SESSION['msg'] = 'Customer tidak ditemukan!';
        echo "<meta http-equiv='refresh' content='0;{$halaman_saat_ini}'>";
        return;
    }
    ...
    $old_foto_profil = $customer['path_to_foto_profil'];
    ...
    mysqli_stmt_prepare($stmt_update, "UPDATE tbl_customer SET path_to_foto_profil=? WHERE id_customer=?");
    mysqli_stmt_bind_param($stmt_update, 'si', $nama_berkas, $id_customer);
